function FromIntToActions(number: number): string | undefined {
    if (number < 1 || number > 31) {
        return;
    }
    let binary_number: number[] = [];
    while (number != 0) {
    ...
    let result: string = "";
    let actions: string[] = ["saltar", "levantar las cejas", "mover la nariz", "parpadear dos veces", "parpadear"];
    for (let i = 5; i >= 0; --i) {
