function RomanToDecimal(roman_number: string): number | undefined {
    const roman_dict = {
        "I": 1,
        "V": 5,
        "X": 10,
    ...
    const InvalidRomanCombinations: string[] = [
        // Repeticiones prohibidas
        "IIII", "VV", "XXXX", "LL", "CCCC", "DD", "MMMM",
    
        // Sustracciones inválidas (longitud 2)
        "IL", "IC", "ID", "IM",  // 'I' no puede restar a L, C, D, M
        "VX", "VL", "VC", "VD", "VM",  // 'V' no puede restar a X, L, C, D, M
        "XD", "XM",  // 'X' no puede restar a D, M
        "LC", "LD", "LM",  // 'L' no puede restar a C, D, M
        "DM",  // 'D' no puede restar a M
    
        // Combinaciones inválidas de longitud 3
        "IVI", "IXI",  // 'I' después de V/X no es válido
        "XLX", "XCX",  // 'X' después de L/C no es válido
        "CDC", "CMC",  // 'C' después de D/M no es válido
        "IIX", "IIV", "IXX",  // Sustracciones múltiples inválidas
        "XXL", "XXC", "CCD", "CCM",  // Combinaciones como XXL (no válido)
        "VIV", "VIX", "VXL", "VXC",  // Combinaciones con 'V' inválidas
        "XLVX", "XLXL", "XCXC",  // Combinaciones repetidas inválidas
        "IXL", "IXC", "IXD", "IXM",  // 'IX' seguido de símbolos mayores
        "XCD", "XCM",  // 'X' no puede restar antes de C, D, M
    ...
    let same_symbol_iterator: number = 1;
    ...
    for (let i = 0; i < roman_number.length; ++i) {
        ...
        let actual_number: number = roman_dict[roman_number[i]];
        let next_number: number = roman_dict[roman_number[i+1]];
...
function DecimalToRoman(decimal_number: number):string | undefined {
    ...
    let result: string = "";
    const index_m: number = Math.trunc(decimal_number / 1000);
    if (index_m != 0) {
        result = result + millar[index_m];
    }
    decimal_number = decimal_number % 1000;
    const index_c: number = Math.trunc(decimal_number / 100);
    if (index_c != 0) {
        result = result + centenas[index_c];
    }
    decimal_number = decimal_number % 100;
    const index_d: number = Math.trunc(decimal_number / 10);
    if (index_d != 0) {
        result = result + decenas[index_d];
    }
    decimal_number = decimal_number % 10;
    const index_u: number = decimal_number;
    if (index_u != 0) {
