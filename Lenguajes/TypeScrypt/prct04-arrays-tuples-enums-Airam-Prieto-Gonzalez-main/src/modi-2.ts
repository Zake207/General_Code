 ...
 */
export function MeshArray(words: string[]): [boolean, string | undefined] {
  let result: [boolean, string] = [false, undefined];
  if (words.length === 0 || words.length === 1) {
    return result;
  }
  let any_dont: boolean = true;
  let concatenation: string = "";
  for (let i = 1; i < words.length; ++i) {
    let pivot_concatenation: string = "";
